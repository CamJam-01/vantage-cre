/** CoStar land-sale export headers and their land_sales column names. */

const COSTAR_HEADER_ROW =
...
/** Columns that already exist as typed land_sales fields — do not also add as text. */
export const COSTAR_TYPED_COLUMNS = ['property_type', 'sale_price', 'sale_date'] as const;
...
export function slugifyHeader(header: string): string {
  let slug = header.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '').slice(0, 63);
  if (!slug || /^[0-9]/.test(slug) || PG_RESERVED.has(slug)) slug = `field_${slug}`;
  return slug;
}
...
export function costarFields(headers: readonly string[] = COSTAR_HEADERS): ReadonlyArray<{ header: string; column: string }> {
  ...
  return headers.map(header => {
    let column = slugifyHeader(header);
    const n = (seen.get(column) ?? 0) + 1;
    seen.set(column, n);
    if (n > 1) column = `${column.slice(0, 60)}_${n}`;
    return { header, column };
  });
}

export function costarTextColumns(headers: readonly string[] = COSTAR_HEADERS): string[] {
  const typed = new Set<string>(COSTAR_TYPED_COLUMNS);
  return costarFields(headers).filter(field => !typed.has(field.column)).map(field => field.column);
}

/** CoStar headers that also populate the app's existing core land_sales fields. */
