...
**Field tags are derived, never listed.** `mergeTagName` in `lib/land-sales/merge-tags.ts` lowercases a catalog header and collapses non-alphanumeric runs to underscores — `Buyer (True) Company` → `{{ buyer_true_company }}`. The catalog's 277 distinct headers produce 277 distinct field tags, asserted by a test. This is §3.1's "never hand-maintain a second list of field names" applied to templates: do not add a curated field-tag list, an alias, or a per-field formatting exception keyed by header name.

**`{{ comp_number }}` is the sole merge-only tag.** It is an explicit descriptor outside the field catalog and resolves to each record's 1-based position in the selected merge order. It resets for every generated document and must never enter storage, database queries, import/export, field configuration, or `COSTAR_HEADER_ROW`. Any additional computed tag is a README §5 scope decision, not an invitation to generalize a second field model.

**Output Flows are the user-facing merge choice.** `docx_output_flows` stores the name and required default template; ordered rows in `docx_output_flow_rules` store exact catalog field names, `contains` / `equals` / `does_not_equal`, test text, and a routed template. Conditions are case-insensitive text comparisons, evaluated in position order with first match winning. The default handles every non-match. Flow evaluation belongs in pure `lib/land-sales/output-flows.ts`, never as conditionals in the route or component. Both tables are global Admin configuration: active users read them, active Admins mutate them, and every template reference uses `on delete restrict`.
...
**Structure rules for the combined document.** The body-level `<w:sectPr>` is page setup and must survive exactly once, at the end; records are separated by a page-break paragraph. For a single template, every other package part is repacked untouched. For a routed merge, the Output Flow's default template owns package-wide page setup, headers, footers, styles, numbering, themes, fonts, and relationships; alternate templates contribute body content only and must have compatible supporting parts. `mergeRoutedDocx` checks compatibility and fails visibly rather than emitting broken references. Headers and footers fill from the first selected record, regardless of which body template it routes to.

**Templates and Output Flows are Admin-managed and global.** Template rows live in `docx_templates`, with files in the private `docx-templates` bucket keyed by row id so a rename never moves a file. That bucket restricts `allowed_mime_types` to the `.docx` type alone, so uploads must pass `contentType` explicitly — a browser reporting `application/octet-stream` is otherwise rejected. Output Flow rule replacement is atomic through `save_docx_output_flow`; do not replace it with a delete-then-insert sequence that can strand a partially saved flow.
...
- **The feedback widget fails silently, twice over.** Its loader configures itself from `document.currentScript.dataset` and simply returns if the site key is missing, and its badge is only appended after a config request that the vendor rejects from any origin not registered against that site key. **Only `https://vantagecre.camjetton.com` is registered, and deliberately so** — the widget does not render on localhost or on Vercel preview URLs, and no badge there is expected rather than a broken tag. Keep the `data-site-key` and `data-position` attributes on the script element itself; a post-load configuration call is not available.
- **The name prefill goes through the widget's public API, not its DOM.** `components/feedback/feedback-name-prefill.tsx` awaits `window.ClientFeedback.ready()` and calls `prefill({ name })`; the widget then re-applies it on open and after each successful send, and leaves a name the visitor edited alone. Do not reintroduce the earlier shadow-DOM approach — reaching into `#submitter-name` now fights the widget's own prefill state. The global is defined by a script injected after hydration, so anything calling it has to wait for it to appear.
