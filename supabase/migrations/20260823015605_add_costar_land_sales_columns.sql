-- Restore the app's core land_sales columns (the live table currently only has
-- id) and add every CoStar export header as a nullable text column. Duplicate
-- "Sprinklers" becomes sprinklers and sprinklers_2. Property Type / Sale Price /
-- Sale Date already exist as typed columns, so those three are not also added
-- as text. Nothing is required.
alter table public.land_sales
  ...
  add column if not exists square_feet numeric(14,2),
  add column if not exists acreage numeric(12,4),
  add column if not exists sale_date date,
  add column if not exists sale_date_raw text,
...
begin
  if array_length(headers, 1) is distinct from 278 then
    raise exception 'expected 278 CoStar headers, got %', array_length(headers, 1);
  end if;

  foreach header in array headers loop
    slug := trim(both '_' from regexp_replace(lower(header), '[^a-z0-9]+', '_', 'g'));
    slug := left(slug, 63);
    if slug = '' or slug ~ '^[0-9]' or slug = any(reserved) then
      slug := 'field_' || slug;
    end if;
    n := coalesce((seen ->> slug)::int, 0) + 1;
    seen := jsonb_set(seen, array[slug], to_jsonb(n));
    col := case when n > 1 then left(slug, 60) || '_' || n::text else slug end;
    if col = any(typed) then
      execute format('comment on column public.land_sales.%I is %L', col, header);
      continue;
    end if;
    execute format('alter table public.land_sales add column if not exists %I text', col);
    execute format('comment on column public.land_sales.%I is %L', col, header);
  end loop;
end $$;
...
begin
  if not exists (
    ...
    where table_schema = 'public' and table_name = 'land_sales' and column_name = 'price_per_acre'
  ) then
    alter table public.land_sales
      add column price_per_acre numeric(15,2)
      generated always as (case when acreage > 0 then round(sale_price / acreage, 2) end) stored;
  end if;
end $$;
